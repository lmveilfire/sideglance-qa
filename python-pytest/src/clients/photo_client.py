...
from src.utils.types import PhotoDto, PhotoPayload


class LikeResult(TypedDict, total=False):
    totalLikes: int
    newlyLiked: bool
    message: str
...
class PhotoClient:
    ...
    @step()
    def list(self) -> builtins.list[PhotoDto]:
        response = self._api.get_all()
        if not response.ok:
            raise RuntimeError(f"[PhotoClient] list failed: {response.status_code} {response.text}")
        return cast("builtins.list[PhotoDto]", response.json())

    @step()
    def list_by_category(self, category_id: int) -> builtins.list[PhotoDto]:
    ...
    def get_by_id(self, photo_id: int) -> PhotoDto:
        ...
        return cast(PhotoDto, response.json())
    ...
    def like(self, photo_id: int) -> LikeResult:
        ...
        return cast(LikeResult, response.json())
