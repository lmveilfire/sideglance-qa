...
test.describe('POST /comments — негативные сценарии', () => {
  ...
  test('TC-COM-06: повторный sessionId: первый 201, второй 429', async ({
    ...
    const first = await commentApi.create(generate.commentData(photo.id), captcha);
    expect(first.status()).toBe(HTTP.CREATED);
    const firstBody = await first.json() as { id: number };
    cleanup.register(() => adminCommentClient.delete(firstBody.id));

    const second = await commentApi.create(generate.commentData(photo.id), captcha);
