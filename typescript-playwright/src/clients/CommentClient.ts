...
import type { CommentDto, CaptchaData, CaptchaResponse, CommentsPageResponse, CommentPayload } from '../utils/types.ts';
...
export class CommentClient {
  ...
  async getCaptcha(): Promise<CaptchaResponse> {
    ...
    if (!statusIn(HTTP.OK)(response.status())) {
      throw new Error(`[CommentClient] getCaptcha failed: ${response.status()} ${await response.text()}`);
    }
  ...
  async listByPhoto(photoId: number, page = 0, size = 5): Promise<CommentsPageResponse> {
    ...
    if (!statusIn(HTTP.OK)(response.status())) {
      throw new Error(`[CommentClient] listByPhoto failed: ${response.status()} ${await response.text()}`);
    }
  ...
  async create(payload: CommentPayload, captcha: CaptchaData): Promise<CommentDto> {
    ...
    if (!statusIn(HTTP.OK, HTTP.CREATED)(response.status())) {
      throw new Error(`[CommentClient] create failed: ${response.status()} ${await response.text()}`);
    }
  ...
  async createInIsolation(payload: CommentPayload, captcha: CaptchaData): Promise<CommentDto> {
    const response = await this.api.create(payload, captcha, { 'X-Forwarded-For': generate.ip() });
    if (!statusIn(HTTP.OK, HTTP.CREATED)(response.status())) {
      throw new Error(`[CommentClient] createInIsolation failed: ${response.status()} ${await response.text()}`);
    }
    return response.json() as Promise<CommentDto>;
  }
}