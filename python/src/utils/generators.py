...
from src.utils.models import CategoryPayload, CommentPayload, PhotoPayload
...
class Generate:
    ...
    def photo_data(**overrides: Any) -> PhotoPayload:
        ...
        return PhotoPayload(**{**base, **overrides})
    ...
    def comment_data(photo_id: int, **overrides: Any) -> CommentPayload:
        ...
        return CommentPayload(**{**base, **overrides})

    @staticmethod
    def category_data(**overrides: Any) -> CategoryPayload:
        base: dict[str, Any] = {"name": f"TEST-{'-'.join(_fake.words(nb=2)).lower()}"}
        return CategoryPayload(**{**base, **overrides})
